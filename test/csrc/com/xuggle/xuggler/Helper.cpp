    ...
    for (i = 0; i < 2; i++)
    {
      ...
      if (codecStr && *codecStr)
      {
        ...
        } else if (newcodec->getType() == ICodec::CODEC_TYPE_AUDIO)
        {
            ...
            if (newcodec->getID() == ICodec::AV_CODEC_ID_AAC){
                newcoder->setSampleFormat(IAudioSamples::FMT_FLTP);
            }
    ...
    while (h.container->readNextPacket(packet.value()) == 0)
    {
          ...
          hw.first_output_video_stream >= 0)
      {
        ...
        while (offset < packet->getSize())
        {
          ...
          num = ic->getTimeBase();
          VS_TUT_ENSURE_DISTANCE("time base changed", num->getDouble(), h.expected_time_base, .0001);
          offset += retval;
          if (frame->isComplete())
          {
            numFrames++;
